...
export async function POST(request: Request) {
  ...
  if (!body.expression || !body.action) {
    return NextResponse.json({ error: 'Missing "expression" or "action" field' }, { status: 400 });
  }

  const result = await callOpenClaw('cron.add', body);
